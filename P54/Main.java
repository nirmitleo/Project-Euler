...
//        demo.test1();
    ...
    public void solve()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            int p = getStraightFlush(p1);
            int q = getStraightFlush(p2);
            if (p != -1 && q == -1)
            {
                count++;
                continue;
            ...
            int[] pp = getTwoPair(p1);
            int[] qq = getTwoPair(p2);
            if (pp != null && qq == null)
            {
                count++;
                continue;
...
//            System.out.println("hi");
    ...
    private int getHighCard(String[] p)
    {
        int[] cards = getCards(p);
        for (int i = 13; i >= 1; i--)
        {
            if (cards[i] > 0)
            {
    ...
    private int getOnePair(String[] p)
    {
        int[] cards = getCards(p);
        for (int i = 13; i >= 1; i--)
        {
    ...
    private int[] getTwoPair(String[] p)
    {
        int[] cards = getCards(p);
        int[] two = new int[2];
        int count = 0;
    ...
    private int getThreeOfKind(String[] p)
    {
        int[] cards = getCards(p);
        for (int i = 13; i >= 1; i--)
        {
    ...
    public void test()
    {
        String[] p1 = new String[]{"1C", "2C", "3C", "4C", "5C"};
        System.out.println(getStraight(p1));
    }

    private int getStraight(String[] p)
    {
        int[] cards = getCards(p);
    ...
    private boolean isFlush(String[] p)
    {
        char suit = p[0].charAt(1);
        for (int i = 1; i < 5; i++)
        {
            if (suit != p[i].charAt(1))
            {
                return false;
            }
        }
    ...
    private boolean isFullHouse(String[] p)
    {
        return getThreeOfKind(p) != -1 && getTwoPair(p) != null;
    }

    private int getFourOfKind(String[] p)
    {
    ...
    private int getStraightFlush(String[] p)
    {
        int s = getStraight(p);
        if (isFlush(p) && s != -1)
        {
    ...
    private boolean isRoyalFlush(String[] p)
    {
        if (!isFlush(p))
        {
            return false;
